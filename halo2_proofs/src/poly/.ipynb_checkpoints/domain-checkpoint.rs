...
impl<F: WithSmallOrderMulGroup<3>> EvaluationDomain<F> {
    ...
    pub fn extended_from_vec(&self, values: Vec<F>) -> Polynomial<F, ExtendedLagrangeCoeff> {
        assert_eq!(values.len(), self.extended_len());
    ...
    pub fn extended_to_coeff_wo_trunc(&self, mut a: Polynomial<F, ExtendedLagrangeCoeff>) -> Vec<F> {
        assert_eq!(a.values.len(), self.extended_len());
        ...
        // it always lies on a power-of-two boundary.
        a.values
            .truncate((&self.n * (self.quotient_poly_degree + 1)) as usize);
