...
use halo2curves::bn256::{Fq2, Fq6, Fq12};
...
// Prefix to a prover's message containing a scalar on the base field
const BLAKE2B_PREFIX_BASE: u8 = 2;

// Prefix to a prover's message containing a scalar on the scalar field
const BLAKE2B_PREFIX_SCALAR: u8 = 3;

// Prefix to a prover's message containing a GT point
const BLAKE2B_PREFIX_GT: u8 = 4;


// Generic transcript (from either the prover or verifier's perspective)
pub trait Transcript<C: CurveAffine, E: EncodedChallenge<C>> {
    ...
    fn squeeze_challenge_base<T>(&mut self) -> ChallengeBase<C, T> {
        ChallengeBase {
            inner: self.squeeze_challenge().get_base(),
            _marker: PhantomData,
        }
    }
    ...
    // treating it as a common input.
    fn common_base(&mut self, scalar: C::Base) -> io::Result<()>;    
    
    // Writing the scalar to the transcript without writing it to the proof,
    // treating it as a common input.
    fn common_scalar(&mut self, scalar: C::Scalar) -> io::Result<()>;
    
    // Only Specialized for Cougar
    fn common_gt(&mut self, point: Gt) -> io::Result<()>;
}
...
pub trait TranscriptRead<C: CurveAffine, E: EncodedChallenge<C>>: Transcript<C, E> {
    ...
    /// Read a curve base from the prover.
    fn read_base(&mut self) -> io::Result<C::Base>;    
    
    /// Read a curve scalar from the prover.
    fn read_scalar(&mut self) -> io::Result<C::Scalar>;
    
    // Read Gt Point from the prover.
    // only for cougar
    fn read_gt(&mut self) -> io::Result<Gt>;
}
...
pub trait TranscriptWrite<C: CurveAffine, E: EncodedChallenge<C>>: Transcript<C, E> {
    ...
    /// Write a curve base from the prover.
    fn write_base(&mut self, base: C::Base) -> io::Result<()>;        
    ...
    // Write Gt Point
    // only for cougar
    fn write_gt(&mut self, point: Gt) -> io::Result<()>;
}
    ...
    C::Base: FromUniformBytes<64>,
{
    ...
    fn read_gt(&mut self) -> io::Result<Gt> {
        let mut c1 = <Fq2 as PrimeField>::Repr::default();
        self.reader.read_exact(c1.as_mut())?;
        let mut c2 = <Fq2 as PrimeField>::Repr::default();
        self.reader.read_exact(c2.as_mut())?;
        let mut c3 = <Fq2 as PrimeField>::Repr::default();
        self.reader.read_exact(c3.as_mut())?;
        let mut c4 = <Fq2 as PrimeField>::Repr::default();
        self.reader.read_exact(c4.as_mut())?;
        let mut c5 = <Fq2 as PrimeField>::Repr::default();
        self.reader.read_exact(c5.as_mut())?;
        let mut c6 = <Fq2 as PrimeField>::Repr::default();
        self.reader.read_exact(c6.as_mut())?;
        let c_l = Fq6 {
            c0: Fq2::from_repr(c1).unwrap(),
            c1: Fq2::from_repr(c2).unwrap(),
            c2: Fq2::from_repr(c3).unwrap()
        };
        let c_r = Fq6 {
            c0: Fq2::from_repr(c4).unwrap(),
            c1: Fq2::from_repr(c5).unwrap(),
            c2: Fq2::from_repr(c6).unwrap()
        };
        let point: Gt = Gt::from_fq12(Fq12{c0: c_l, c1: c_r});
        self.common_gt(point)?;
        Ok(point)
    }
}
    ...
    C::Base: FromUniformBytes<64>,
{
    ...
    fn common_base(&mut self, base: C::Base) -> io::Result<()> {
        self.state.update(&[BLAKE2B_PREFIX_BASE]);
        self.state.update(base.to_repr().as_ref());
    ...
    fn common_gt(&mut self, point: Gt) -> io::Result<()> {
        self.state.update(&[BLAKE2B_PREFIX_GT]);
        let inner: Fq12 = point.to_fq12();
        let (c0, c1) = (inner.c0, inner.c1);
        self.state.update(c0.c0.to_repr().as_ref());
        self.state.update(c0.c1.to_repr().as_ref());
        self.state.update(c0.c2.to_repr().as_ref());
        self.state.update(c1.c0.to_repr().as_ref());
        self.state.update(c1.c1.to_repr().as_ref());
        self.state.update(c1.c2.to_repr().as_ref());
        Ok(())   
    }
}
    ...
    C::Base: FromUniformBytes<64>,
{
    ...
    fn write_gt(&mut self, point: Gt) -> io::Result<()> {
        ...
        let inner: Fq12 = point.to_fq12();
        let (c0, c1) = (inner.c0, inner.c1);
        self.writer.write_all(c0.c0.to_repr().as_ref())?;
        self.writer.write_all(c0.c1.to_repr().as_ref())?;
        self.writer.write_all(c0.c2.to_repr().as_ref())?;
        self.writer.write_all(c1.c0.to_repr().as_ref())?;
        self.writer.write_all(c1.c1.to_repr().as_ref())?;
        self.writer.write_all(c1.c2.to_repr().as_ref())    
    }
}
...
#[derive(Copy, Clone, Debug)]
pub struct ChallengeBase<C: CurveAffine, T> {
    inner: C::Base,
    _marker: PhantomData<T>,
}

impl<C: CurveAffine, T> std::ops::Deref for ChallengeBase<C, T> {
    type Target = C::Base;
...
pub trait EncodedChallenge<C: CurveAffine> {
    ...
    fn as_challenge_base<T>(&self) -> ChallengeBase<C, T> {
        ChallengeBase {
            inner: self.get_base(),
            _marker: PhantomData,
        }
...
pub(crate) fn read_n_bases<C: CurveAffine, E: EncodedChallenge<C>, T: TranscriptRead<C, E>>(
    transcript: &mut T,
    n: usize,
) -> io::Result<Vec<C::Base>> {
    (0..n).map(|_| transcript.read_base()).collect()
}
...
pub(crate) fn read_n_gts<C: CurveAffine, E: EncodedChallenge<C>, T: TranscriptRead<C, E>> (
    transcript: &mut T, n: usize
) -> io::Result<Vec<Gt>> {
    (0..n).map(|_| transcript.read_gt()).collect()
}