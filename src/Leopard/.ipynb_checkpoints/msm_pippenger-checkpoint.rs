...
pub trait Pippenger {
    fn pippenger<P, I>(points:P, scalars: I) -> Self
        where P: Iterator<Item = Self>, I: Iterator<Item = Scalar>;
    
    fn mul_by_pow_2(point: &Self, k: u32) -> Self;
}

impl Pippenger for Gt {
    fn pippenger<P, I>(points: P, scalars: I) -> Self
    where
        P: Iterator<Item = Self>,
        I: Iterator<Item = Scalar>,
    {
        ...
        columns.fold(hi_column, |total, p| Self::mul_by_pow_2(&total, w as u32) + p)
    }
...
pub fn to_radix_2w_size_hint(w: usize) -> usize {
    debug_assert!(w >= 6);
    debug_assert!(w <= 8);
...
pub fn to_radix_2w(scalar: &Scalar, w: usize) -> [i8; 43] {
    debug_assert!(w >= 6);
    debug_assert!(w <= 8);
    ...
    LittleEndian::read_u64_into(&scalar.to_bytes_le(), &mut scalar64x4[0..4]);
